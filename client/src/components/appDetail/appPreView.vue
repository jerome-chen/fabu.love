    ...
    <div :class="getContentClass()" v-if="this.appBaseData">
      <div :class="getLeftClass()">
        ...
        <div class="pcWrapper" v-show="!isPhone">
          ...
          <div class="info">
            <p
              v-if="this.appVersionInfo.versionStr"
              class="desc"
            >版本：{{this.appVersionInfo.versionStr}}</p>
          ...
          <el-button
            ...
            round
          >
            <i :class="this.platformStr === 'ios' ? 'icon-ic_ios':'icon-ic_andr'"></i> 下载安装
          </el-button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    showDownLoadBtn() {
      ...
      if (p.indexOf("Mac") === 0) {
        ...
        if (
          this.appBaseData.installWithPwd !== 1 ||
          this.pwd === this.appBaseData.installPwd
        ) {
          return true;
        } else {
          return false;
  ...
  methods: {
    ...
    getAppInfo(shortUrl) {
      AppResourceApi.getAppInfoByShortUrl(shortUrl).then(
        res => {
          console.log(res);
          ...
          this.appVersionInfo.creatDateStr = `${releaseDate.getFullYear()}-${releaseDate.getMonth() +
            1}-${releaseDate.getDate()}`;
          ...
          reader.read().then(function processResult(result) {
            if (result.done) {
              ...
              AppResourceApi.downloadedCount(
                _this.appBaseData._id,
                _this.appVersionInfo._id
              ).then(() => {}, reject => {});
              return;
            }
